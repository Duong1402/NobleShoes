...
const validateForm = () => {
  ...
  if (!ten) {
    notify.warning("Vui lòng nhập tên chất liệu!");
    return false;
  }

  if (ten.length < 3) {
    notify.warning("Tên chất liệu phải có ít nhất 3 ký tự!");
    return false;
  }
  ...
  if (duplicate) {
    notify.warning("Tên chất liệu đã tồn tại, vui lòng chọn tên khác!");
    return false;
  }
...
const saveItem = async () => {
  try {
    ...
    if (selectedDayGiay.value.id) {
      // Cập nhật
      await updateChatLieu(selectedDayGiay.value.id, selectedDayGiay.value);
      notify.success("Cập nhật thành công!");
    } else {
      // Thêm mới: Gửi đối tượng không có mã (mã sẽ được backend tạo)
      await createChatLieu(selectedDayGiay.value);
  ...
  <div class="container-fluid mt-4">
    <div class="card shadow-sm border-0 mb-4">
      <div class="card-body py-2 px-3">
        ...
        >
          <div>
            <h3 class="fw-bold text-warning mb-1">Quản lý Chất Liệu</h3>
    ...
    <div class="card">
      <div class="card-header">
        <h4 class="card-title mb-0">
          <i class="fa fa-list me-2"></i>Danh sách chất liệu
        </h4>
      </div>
      <div class="card-body">
        ...
        <div class="modal-content">
          <div class="modal-header bg-warning text-white">
            <h5 class="modal-title" id="detailModalLabel">
              {{
                selectedDayGiay.id ? "Cập nhật Chất liệu" : "Thêm mới Chất liệu"
              }}
            </h5>
            <button
              type="button"
            ...
            <div class="row g-3">
              <div class="col-12" v-if="selectedDayGiay.id">
                ...
                <small class="text-muted"
                  >Mã được tạo tự động và không thể thay đổi.</small
                >
              </div>
