...
const overviewStats = ref({
  HOM_NAY: { title: "Hôm nay", total: "0 ₫", sanPham: 0, donThanhCong: 0, donHuy: 0, donTra: 0, colorClass: "bg-info", icon: "fa-regular fa-calendar-check" },
  TUAN_NAY: { title: "Tuần này", total: "0 ₫", sanPham: 0, donThanhCong: 0, donHuy: 0, donTra: 0, colorClass: "bg-warning", icon: "fa-regular fa-calendar-minus" },
  THANG_NAY: { title: "Tháng này", total: "0 ₫", sanPham: 0, donThanhCong: 0, donHuy: 0, donTra: 0, colorClass: "bg-primary", icon: "fa-calendar-alt" },
  NAM_NAY: { title: "Năm nay", total: "0 ₫", sanPham: 0, donThanhCong: 0, donHuy: 0, donTra: 0, colorClass: "bg-success", icon: "fa-calendar" },
  TUY_CHINH: { title: "Tùy chỉnh", total: "0 ₫", sanPham: 0, donThanhCong: 0, donHuy: 0, donTra: 0, colorClass: "bg-info", icon: "fa-edit" }
});
...
const mapOverviewDto = (dto) => {
  if (!dto) return { total: "0 ₫", sanPham: 0, donThanhCong: 0, donHuy: 0, donTra: 0 };
  return {
...
const loadOverviewData = async () => {
  ...
  try {
    ...
    overviewStats.value.HOM_NAY = { ...overviewStats.value.HOM_NAY, ...mapOverviewDto(data.homNay) };
    overviewStats.value.TUAN_NAY = { ...overviewStats.value.TUAN_NAY, ...mapOverviewDto(data.tuanNay) };
    overviewStats.value.THANG_NAY = { ...overviewStats.value.THANG_NAY, ...mapOverviewDto(data.thangNay) };
    overviewStats.value.NAM_NAY = { ...overviewStats.value.NAM_NAY, ...mapOverviewDto(data.namNay) };
...
const loadFilteredData = async () => {
  ...
  try {
    ...
    if (data.thongKeTuyChinh) {
      overviewStats.value.TUY_CHINH = { ...overviewStats.value.TUY_CHINH, ...mapOverviewDto(data.thongKeTuyChinh) };
    }
...
watch([lowStockCurrentPage, lowStockItemsPerPage], () => {
    loadOverviewData();
}, { immediate: false });

watch([selectedFilter, customStartDate, customEndDate, currentPage, itemsPerPage], () => {
    loadFilteredData();
}, { immediate: false }); 
...
const handleExportThongKeExcel = async () => {
  try {
    ...
    const currentStat = overviewStats.value[filter] || overviewStats.value.TUY_CHINH;
...
<template>
  ...
  <button 
    ...
    @click="selectFilter('NGAY')"> NGÀY
  </button>
  <button 
    type="button" 
    class="btn" 
    :class="selectedFilter === 'TUẦN' ? 'btn-warning' : 'btn-outline-dark'" 
    @click="selectFilter('TUAN')"> TUẦN
  </button>
  <button 
    type="button" 
    class="btn" 
    :class="selectedFilter === 'THÁNG' ? 'btn-warning' : 'btn-outline-dark'" 
    @click="selectFilter('THANG')"> THÁNG
  </button>
  <button 
    type="button" 
    class="btn" 
    :class="selectedFilter === 'NĂM' ? 'btn-warning' : 'btn-outline-dark'" 
    @click="selectFilter('NAM')"> NĂM
  </button>
  <button 
  type="button" 
  class="btn" 
  :class="selectedFilter === 'TUY_CHINH' ? 'btn-warning' : 'btn-outline-dark'" 
  @click="selectFilter('TUY_CHINH')"> TÙY CHỈNH
</button>
</div>
            <div v-if="selectedFilter === 'TUY_CHINH'" class="d-flex flex-wrap gap-2">
          ...
          <div>
            <button class="btn btn-outline-success" @click="handleExportThongKeExcel">
            ...
            <div class="table-responsive">
              <table class="table align-middle mb-0">
                ...
                <tbody>
                  ...
                  <tr v-else v-for="product in bestSellingProducts" :key="product.tenSanPham + product.giaTien">
                    <td class="text-center">
                      <img :src="product.hinhAnh || '/src/assets/img/logoWeb.png'" alt="product-img" style="width: 50px; height: 50px; object-fit: cover; border-radius: 4px;">
                    </td>
                    <td>{{ product.tenSanPham }}</td>
                    <td class="text-center">{{ product.soLuongBan }}</td>
                    <td class="text-end fw-bold">{{ formatCurrency(product.giaTien) }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
        ...
        <div class="card shadow-sm border-0 mb-4">
          <div class="card-header d-flex flex-wrap justify-content-between align-items-center">
            ...
            <div class="d-flex align-items-center gap-2">
              ...
              <input type="number" :value="10" class="form-control form-control-sm" style="width: 80px;" disabled>
            </div>
          </div>
          <div class="card-body p-0">
            <div class="table-responsive">
              <table class="table align-middle mb-0">
                ...
                <tbody>
                  ...
                  <tr v-else v-for="product in lowStockProducts" :key="product.tenSanPham + product.giaBan">
                    <td class="text-center">
                      <img :src="product.hinhAnh || '/src/assets/img/logoWeb.png'" alt="product-img" style="width: 50px; height: 50px; object-fit: cover; border-radius: 4px;">
                    </td>
                    <td>{{ product.tenSanPham }}</td>
                    <td class="text-center fw-bold text-danger">{{ product.soLuongTon }}</td>
                    <td class="text-end fw-bold">{{ formatCurrency(product.giaBan) }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
             <div class="card-footer d-flex justify-content-between align-items-center">
              <div class="d-flex align-items-center gap-2">
                <span class="text-muted" style="font-size: 0.9rem;">Xem</span>
                <select class="form-select form-select-sm" v-model="lowStockItemsPerPage" style="width: 75px;">
                  <option value="5">5</option>
                  <option value="10">10</option>
                  <option value="25">25</option>
                </select>
              ...
              <nav v-if="lowStockTotalPages > 0">
                <ul class="pagination pagination-sm mb-0">
                  <li class="page-item" :class="{ disabled: lowStockCurrentPage === 0 }">
                    <a class="page-link" href="#" @click.prevent="changeLowStockPage(lowStockCurrentPage - 1)">&laquo;</a>
                  </li>
                  <li 
                    ...
                    <a class="page-link" href="#" @click.prevent="changeLowStockPage(page - 1)">{{ page }}</a>
                  </li>
                  <li class="page-item" :class="{ disabled: lowStockCurrentPage === lowStockTotalPages - 1 }">
                    <a class="page-link" href="#" @click.prevent="changeLowStockPage(lowStockCurrentPage + 1)">&raquo;</a>
                  </li>
                </ul>
              </nav>
            </div>
              ...
              <div v-for="item in pieChartLegend" :key="item.label" class="col-6 mb-1">
                <div class="d-flex align-items-center justify-content-between">
                  <div class="d-flex align-items-center" style="min-width: 0;">
                    <span :style="{ backgroundColor: item.color, width: '15px', height: '15px', display: 'inline-block', marginRight: '8px', borderRadius: '3px', flexShrink: 0 }"></span>
          ...
          <div class="card-body p-0">
            ...
            <ul v-else class="list-group list-group-flush">
              ...
              >
                ...
                <span class="fw-bold">
                  {{ (stat.label.startsWith('Doanh thu')) ? formatCurrency(stat.value) : stat.value }}
                </span>

                <span :class="stat.trend === 'up' ? 'text-success' : (stat.trend === 'down' ? 'text-danger' : 'text-white')">
                  <i :class="stat.trend === 'up' ? 'fas fa-arrow-up' : (stat.trend === 'down' ? 'fas fa-arrow-down' : 'fas fa-equals')"></i>
                  {{ Math.abs(stat.percentage).toFixed(2) }}%
                </span>
              </li>
            </ul>
          </div>
